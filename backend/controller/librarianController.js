...
export const updateLibrarian = async (req, res) => {
  try {
    const updated = await librarianService.updateLibrarian(req.params.id, req.body);
    res.status(200).json({ message: 'Librarian updated successfully', librarian: updated });
  } catch (error) {
    res.status(error.status || 500).json({ message: error.message || 'Error updating librarian' });
  }
};
