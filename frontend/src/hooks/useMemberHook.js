...
export const useMemberHook = () => {
  ...
  // State handlers that keep search params in sync without triggering cascading effect renders
  ...
  const processLoadedUsers = (memberRes) => {
    ...
    const dbMembers = memberRes.status === "fulfilled" && Array.isArray(memberRes.value?.data)
      ? memberRes.value.data
      : [];

    const normalizeUser = (m, idx) => {
      const dbId = m.id;
      const uId = m.user_id || m.studentId || m.id || `MEM-${idx + 1}`;
      const name = m.name || m.fullName || `${m.first_name || ""} ${m.last_name || ""}`.trim() || "User";
      return {
        ...m,
        db_id: dbId,
        id: uId,
        ...
        name: name,
        email: m.email || `${name.toLowerCase().replace(/\s+/g, ".")}@library.com`,
        role: m.role || "Student",
        department: m.department || "CS",
        registeredDate: m.joined_date || m.created_at || m.registeredDate || new Date().toISOString().split("T")[0],
        status: m.status || "active",
        phone: m.phone || "1234567890",
      };
    };

    const normalizedDb = dbMembers.map((m, idx) => normalizeUser(m, idx));
    const normalizedStored = storedRegistered.map((m, idx) => normalizeUser(m, dbMembers.length + idx));

    const allCombined = [...normalizedDb, ...normalizedStored];
    if (allCombined.length === 0) {
      allCombined.push(...DEFAULT_INITIAL_MEMBERS);
    }
  ...
  const handleAddStudentSubmit = async (dataOrEvent) => {
    ...
    const inputData = dataOrEvent && (dataOrEvent.fullName || dataOrEvent.name) ? dataOrEvent : studentFormData;
    ...
    const payload = {
      ...
      phone: inputData.phone || "1234567890",
      status: (inputData.status || "active").toLowerCase(),
      joined_date: new Date().toISOString().split("T")[0],
    };

    // Save locally first to guarantee UI updates immediately
    const existingRegistered = JSON.parse(localStorage.getItem("registered_users") || "[]");
    const updatedRegistered = [payload, ...existingRegistered];
    localStorage.setItem("registered_users", JSON.stringify(updatedRegistered));
    setStudents((prev) => [payload, ...prev.filter((s) => s.id !== payload.id && s.studentId !== payload.studentId)]);
    ...
    try {
      ...
      console.warn("Add user API notice:", err?.message);
      setStudentFormData({
        name: "",
        studentId: "",
        email: "",
  ...
  const filteredStudents = useMemo(() => {
    return students.filter((s) => {
      ...
      const matchesSearch =
        (s.name && s.name.toLowerCase().includes(searchParamQuery.toLowerCase())) ||
        (s.studentId && s.studentId.toLowerCase().includes(searchParamQuery.toLowerCase())) ||
        (s.email && s.email.toLowerCase().includes(searchParamQuery.toLowerCase()));
      const matchesDept = selectedDept === "All" || s.department === selectedDept;
      const matchesRole = roleParam === "All" || (s.role || "Student") === roleParam;
      return matchesSearch && matchesDept && matchesRole;
    });
  }, [students, searchParamQuery, selectedDept, roleParam]);
