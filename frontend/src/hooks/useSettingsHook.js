...
export const useSettingsHook = () => {
  ...
  const [dbStats, setDbStats] = useState({
    ...
    dbPort: 5432,
    dbUser: "postgres",
    tableCounts: { users: 0, member: 0, book: 0, loan: 0, fine: 0, fine_payment: 0 },
  ...
  const handleLibraryInfoSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: "success", text: "Library information and policies saved to database!" });
    } catch (err) {
      setMessage({ type: "error", text: err.response?.data?.message || "Failed to save library settings." });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePreferencesSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: "success", text: "System preferences updated in database!" });
    } catch (err) {
  ...
  const handleTestDatabase = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setMessage({ type: "success", text: "PostgreSQL database connected & healthy!" });
      }
    } catch (err) {
      setMessage({ type: "error", text: "Failed to connect to database: " + err.message });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleBackupDatabase = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        setMessage({ type: "success", text: res.data.message || "Database backup snapshot generated!" });
        fetchSettings();
      }
    } catch (err) {
      setMessage({ type: "error", text: "Backup operation failed: " + err.message });
    } finally {
      setSaving(false);
    }
  };
